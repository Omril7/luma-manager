type IncomeRow = { final_price: number; discount_amount: number }
...
export default function IncomeSummaryCards({ rows }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <p className="text-sm text-muted-foreground mb-1">מספר הזמנות</p>
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <p className="text-sm text-muted-foreground mb-1">הכנסה נטו</p>
        <p className="text-2xl font-bold text-green-600 dark:text-green-400">{fmt(total)}</p>
      </div>
    </div>
  )
}
